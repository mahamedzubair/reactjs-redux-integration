...
const setRef = {
	'coverage': React.createRef(),
	'care-plan': React.createRef(),
	'care-team': React.createRef(),
	'find-care': React.createRef()
};
...
class Navigation extends Component {
    ...
    componentDidMount() {
        this.props.dispatch(Actions.getNav());
    }

    static getDerivedStateFromProps(newProps, currentState) {
        if (newProps.data !== currentState.data) {
        console.log('newProps navigation', newProps);
	...
	renderNav = (t) => {
		return (
            <Fragment>
                {this.props.data.navData.navigation &&
                    <ul>
                        {this.props.data.navData.navigation.map((list) => this.renderNavList(list))}
                    </ul>
                }
            </Fragment>) 
	}
    ...
    renderNavList = (list) => {
        if(list.subnav.length) {
            return(
                <Fragment>
                    <li onClick={() => this.selectNav(list.key)}>
                        <UIMenu className="text-left"
                            buttonClass={this.state[list.key]}
                            label={list.Title}
                            menuClass="sub-nav"
                            ref={setRef[list.key]}>
                                {list.subnav.map((subList) => <Link data-nofocus to={subList.link}>{subList.title}</Link>)}
                            </UIMenu>
                    </li>
                </Fragment>
            )
